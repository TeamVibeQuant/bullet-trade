...
def get_stock_list(context):
    ...
    for stock in initial_list:
        ...
        is_stock_end = end_date is not None and end_date <= context.current_dt.date()
    ...
    filtered_list = list(get_fundamentals(q).code)
...
def check_high_volume(context):
    ...
    for stock in current_positions:
        ...
        df_hist_volume = get_bars(stock, count=g.HV_duration, unit='1d', fields=['volume'], include_now=False, df=True)
        if curr_volume > g.HV_ratio * df_hist_volume['volume'].values.max():
...
def get_concept_stock_list(context, stock_list, momentum_days=7, top_k_concepts=10):
    ...
    concepts_df = concepts_df[concepts_df['start_date'] <= context.current_dt.date()]
    ...
    stock_momentum = stock_momentum[0]
